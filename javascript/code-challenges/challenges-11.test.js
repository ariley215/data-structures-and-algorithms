...
const longestString = (arr) => {
  ...
  for (let i = 1; i < arr.length; i++) {
    if (arr[i].length > maxLength) {
      // Update the index and maximum length if a longer string is found
      longestIndex = i;
...
const firstLetters = (arr) => {
  const result = arr.map(str => str.charAt(0));
  return result;
};
...
const standardizePhoneNumbers = (arr) => {
  // const result = arr.map(str => str.replace(/\D/g, ''));
  // return result;
  const result = arr.map(str => str.split('').filter(char => '0' <= char && char <= '9').join(''));
  return result;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
const allHappy = (arr) => {
  return arr.reduce((result, str) => result && str.includes(':)'), true);
