...
const count = (target, input) => {
  return input.reduce((count, array) => {
    const presentInArray = array.filter(element => element === target).length;
    return count + presentInArray;
  }, 0);
};
...
const totalSum = (input) => {
  return input.reduce((sum, array) => {
    const sumOfArray = array.reduce((acc, element) => acc + element, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((array) => {
    const filteredArr = array.filter((element) => typeof element === 'number' && element % 5 === 0);
    const powerArr = filteredArr.map((element) => Math.pow(2, element));
    return powerArr;
  });
};
